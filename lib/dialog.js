...
const canvasW = 220 * 2.0;// only modify this for scaling!
const canvasH = canvasW * 140 / 220;
c.width = canvasW;
c.height = canvasH;
    ...
    return new Promise(function(resolve, reject){
        ...
        function update(_time){
            ...
            if(textDone && !optionsShowing){
                ctx.drawImage(arrowImage, (boxW/2 - border*3 + centerX)*sc, (boxH/2 - border*4 + centerY)*sc, arrowImage.width*sc, arrowImage.height*sc)
            }
            ...
            if (optionsShowing){
                ...
                roundRect(5*sc,10*sc,(w+23)*sc,(options.length * 15+10)*sc, 10*sc)
                ctx.fillStyle = 'black'
                ctx.strokeStyle='white'
                options.forEach((s,idx)=>{
                    ctx.strokeText(s,20*sc,(idx*15 + 25)*sc)
                    ctx.fillText(s,20*sc,(idx*15 + 25)*sc)
                })
                ctx.drawImage(arrowImage, 10*sc, (optionPos*15+18)*sc, arrowImage.width*sc, arrowImage.height*sc)
                ctx.restore()
            }
