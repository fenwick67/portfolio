...
import {loadOutfit, loadSkin} from './unlocks'
...
class Player{
    ...
    constructor(armature, animationRoot, animations){
        {
            ...
            this.lastAnimSpeed = 0.0;
            // need to setTimeout so these won't happen until entites are loaded (Player is initialized before entities are totally ready)
            setTimeout(loadOutfit,0);
            setTimeout(loadSkin,0);
    ...
    setMoveAnimationForSpeed(speed){
        ...
        // if the cached value hasn't changed (much), then don't update animation weights
        // because it's a little expensive
        if (Math.abs(this.animSpeed  - this.lastAnimSpeed) < 0.001){
            return;
        }else{
            this.lastAnimSpeed = this.animSpeed;
        }
    ...
    update(delta){
        ...
        if (!this.interactingWith){
            ...
            var camAngle = new Vector3();
            playerCam.camera.getWorldDirection(camAngle)
            var angle = Math.atan2(-camAngle.x, -camAngle.z);
            this.velocity.applyAxisAngle(UP, angle)
            if (input.movementAxes.length() > 0.1){
                this.armature.setRotationFromAxisAngle(UP, Math.atan2(this.velocity.x,this.velocity.z))
            }
            if (window.debug && input.buttons.speedydebug){
